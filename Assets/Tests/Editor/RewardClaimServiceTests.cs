...
{
    public sealed class RewardClaimServiceTests
    {
        [Test]
        public async Task ClaimAsync_ReturnsNull_WhenSessionIsMissing()
        {
            ...
            Reward[] rewards = await service.ClaimAsync();

            Assert.That(rewards, Is.Null);
            Assert.That(childrenApiClient.ClaimRewardsCallCount, Is.EqualTo(0));
            Assert.That(syncService.ReloadCurrentProfileCallCount, Is.EqualTo(0));
        }

        [Test]
        public async Task ClaimAsync_ReturnsNull_WhenCurrentProfileIsMissing()
        {
        ...
        [Test]
        public async Task ClaimAsync_ReturnsNull_WhenApiReturnsNull()
        {
            ClientGameStateStore store = CreateStore(CreateProfile("child-7"));
            FakeChildrenApiClient childrenApiClient = new FakeChildrenApiClient
            {
                ClaimRewardsResult = null
            };
            FakeChildGameStateSyncService syncService = new FakeChildGameStateSyncService();
            RewardClaimService service = new RewardClaimService(
                store,
            ...
            Assert.That(rewards, Is.Null);
            Assert.That(childrenApiClient.ClaimRewardsCallCount, Is.EqualTo(1));
            Assert.That(childrenApiClient.LastClaimedProfileId, Is.EqualTo("child-7"));
            Assert.That(syncService.ReloadCurrentProfileCallCount, Is.EqualTo(0));
        }

        [Test]
        public async Task ClaimAsync_ReloadsCurrentProfile_WhenClaimSucceeds()
        {
            ...
            ClientGameStateStore store = CreateStore(CreateProfile("child-9"));
            FakeChildrenApiClient childrenApiClient = new FakeChildrenApiClient
            {
                ClaimRewardsResult = expectedRewards
            };
            FakeChildGameStateSyncService syncService = new FakeChildGameStateSyncService();
            RewardClaimService service = new RewardClaimService(
                store,
            ...
            Assert.That(rewards, Is.SameAs(expectedRewards));
            Assert.That(childrenApiClient.ClaimRewardsCallCount, Is.EqualTo(1));
        ...
        {
            public Reward[] ClaimRewardsResult { get; set; }

            public int ClaimRewardsCallCount { get; private set; }

            public string LastClaimedProfileId { get; private set; }
            ...
            public Task<Reward[]> ClaimRewardsAsync(string remoteProfileId)
            {
                ...
                LastClaimedProfileId = remoteProfileId;
                return Task.FromResult(ClaimRewardsResult);
            }
